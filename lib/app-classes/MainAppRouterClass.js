...
const MainAppRouter = MainAppClass.subClass(function() {
    ...
    instance.doubleReloadResponse = instance.getProps().__appProps['double-reload-response'];
    if (instance.doubleReloadResponse) {
        instance.resetDelayTimer();
        // create listener to app:reloadprops, which might cancel the reload, if it is triggered by the "not operational" delayedResponse
        instance.before('app:reloadprops', e => {
            if (e.delayedResponse!==instance.delayedResponse) {
                e.halt();
            }
        });
    }
...
},
{
    resetDelayTimer: function() {
        const instance = this;
        if (instance.doubleReloadResponse) {
            instance.delayedResponse = instance.getProps().__appProps['delayed-response'];
        }
    },

    setDelayTimer: function(delayed) {
        this.delayedResponse = !!delayed;
    },
