...
const extend = async (server, options, appConfig, viewComponentCommon, viewComponentNrs, appTitles, clientRoutes, startupTime) => {
    ...
    */
    ...
    server.decorate('reply', 'assets', function(filename) {
        let prefix = cwd+buildPrefix+'public/assets/',
            reply = this;
        reply.file(Path.join(prefix, filename));
    });

    // DO NOT use arrowfunction here: we need the former context
    server.decorate('reply', 'reactview', async function(view, config) { // NO ARROW FUNCTION --> we need `this` as it is set (===reply)
        const reply = this,
            request = reply.request;
        let modelConfig, props, affinityView, data, onlyProperties, sendRequireId, file404, generalModelFn,
            generalModel, replaceObject, isStringMessage, offLineMsgField;
        ...
        try {
            ...
            sendRequireId = !!config.__sendRequireId__;
            ...
            try {
                ...
                generalModel = await generalModelFn(request, reply, modelConfig, props.__appProps.lang, appConfig);
            ...
            // merge view-model:
            await modelHandler.merge(request, reply, props, modelConfig, appConfig, view);
