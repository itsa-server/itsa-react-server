...
const merge = async (request, h, props, routeOptions, appConfig, view) => {
    ...
    const getModelFn = async level => {
        ...
        promise = new Promise(function(resolve) {
            try {
                ...
                GLOBAL_MODELS[modelAffinity] = modelFn;
                resolve(modelFn);
            }
            catch (err) {
    ...
    if (modelFn) {
        try {
            ...
            modelProps = await modelFn(request, h, routeOptions || {}, props.__appProps.lang, appConfig);
