    ...
    */
    ...
    server.decorate('toolkit', 'generateProps', async function(view, routeOptions) {
        const reply = this,
            request = reply.request;
        let config={},
            props, affinityView, ajaxProperties, generalModelFn, initialGlobalStateFn, initialGlobalState,
            generalModel, replaceObject, isStringMessage, offLineMsgField, noAuthentication, globalstateConfig;
        ...
        try {
            ...
            if (globalstateConfig && globalstateConfig.enabled) {
                ...
                try {
                    ...
                    initialGlobalState = await initialGlobalStateFn(request, reply, routeOptions, props.__appProps.lang, appConfig);
    ...
    // DO NOT use arrowfunction here: we need the former context
    server.decorate('toolkit', 'reactview', async function(view, routeOptions) { // NO ARROW FUNCTION --> we need `this` as it is set (===reply)
        const reply = this,
            request = reply.request;
        let config={},
            props, affinityView, data, ajaxProperties, ajaxComp, ajaxCss, sendRequireId, file404, generalModelFn, globalstateConfig,
            generalModel, replaceObject, isStringMessage, offLineMsgField, noAuthentication, debugIntro, initialGlobalStateFn, initialGlobalState;
