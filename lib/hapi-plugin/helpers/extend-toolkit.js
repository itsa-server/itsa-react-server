...
const extend = async (server, options, appConfig, viewComponentCommon, viewComponentNrs, appTitles, clientRoutes, startupTime) => {
    ...
    */
    ...
    if (Object.itsa_isObject(appConfig.cookies)) {
        // argument order follows cookieHandler.register(server, propsConfig, notExposedConfig, bodyDataConfig, ...)
        cookieHandler.register(server, appConfig.cookies.props, appConfig.cookies['not-exposed'], appConfig.cookies['body-data-attr'], appConfig.cookies.globalstate, cookiePasswords);
    }
