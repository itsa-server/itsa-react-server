...
const merge = async (context, affinityView, viewComponentCommon, viewComponentNrs, appConfig) => {
    ...
    const getCommonCss = async () => {
        ...
        if (GLOBAL_ASSETS.COMMONCSS!==undefined) {
            ...
            if (appConfig.cdn && appConfig.cdn.enabled) {
                GLOBAL_ASSETS.COMMONCSS = appConfig.cdn.url+'assets/'+appConfig.packageVersion+'/_itsa_server_commons/'+viewComponentCommon.cssfile;
                returnData = GLOBAL_ASSETS.COMMONCSS;
            }
            else {
                // use local file
    ...
    const getCommonScript = async () => {
        ...
        if (GLOBAL_ASSETS.COMMONJS!==undefined) {
            ...
            try {
                ...
                if (appConfig.cdn && appConfig.cdn.enabled) {
                    GLOBAL_ASSETS.COMMONJS = appConfig.cdn.url+'assets/'+appConfig.packageVersion+'/_itsa_server_commons/'+viewComponentCommon.hash+'.js';
                }
                else {
                    // use local file
                    // not yet looked up: search on the disk
    ...
    if (appConfig['babel-polyfill']) {
        context.__appProps.itsapolyfilles6script = ((appConfig.cdn && appConfig.cdn.enabled) ?
            appConfig.cdn.url+'assets/_itsa_server_external_modules/'+findPackageVersion.getVersion(BABEL_POLYFILL)+'/' :
            (Path.resolve('/assets/_itsa_server_external_modules/'+findPackageVersion.getVersion(BABEL_POLYFILL))+'/')) + BABEL_POLYFILL;
    }
    context.__appProps.itsaprebootcapturescript = ((appConfig.cdn && appConfig.cdn.enabled) ?
        appConfig.cdn.url+'assets/_itsa_server_external_modules/'+findPackageVersion.getVersion(PREBOOT_CAPTURE)+'/' :
