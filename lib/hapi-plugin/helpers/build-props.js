...
const generate = (request, view, appTitles, config, appConfig, clientRoutes, startupTime) => {
    let props = {},
        clientLang, affinityTitle, urisplit, authCookie, socketIoClient, sequentialClientUpdate;
    ...
    if (props.__appProps.uri && (props.__appProps.uri.itsa_endsWith('\\?') || props.__appProps.uri.itsa_endsWith('&'))) {
    ...
    if (appConfig.socketServer && appConfig.socketServer.enabled) {
        ...
        props.__appProps.socketport = appConfig.socketServer['proxy-port'] + (process.env.NODE_APP_INSTANCE ? parseInt(process.env.NODE_APP_INSTANCE, 10) : 0); // see https://github.com/Unitech/PM2/issues/1510
        ...
        // `sequentialClientUpdate` is optional in the manifest
        sequentialClientUpdate = appConfig.sequentialClientUpdate || {};
        props.__appProps['double-reload-response'] = (sequentialClientUpdate['update-time'] && sequentialClientUpdate['delay-time']);
        props.__appProps['delayed-response'] = !!sequentialClientUpdate.delay;
    }
