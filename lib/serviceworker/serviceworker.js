...
(function(/* serviceworker, version, urlsToCache, offlineImage, offlinePage, socketPort, cdnUrl */) {
    ...
    var addToCache = function(cacheKey, request, response) {
        ...
        // DO NOT add to cache if the file is a redirect because of `no authentication`
        // This can be seen by the response-header 'x-noauth':
        if (response.headers.get('x-noauth')==='true') {
            console.warn('not added to cache because of no authenticated redirected:', request.url);
            return response;
        }
        copy = response.clone();
        caches.open(cacheKey).then(function(cache) {
            var ajaxProps = url.pathname.startsWith('/_itsa_server_ajax_\/props/'),
            ...
            console.warn('add to cache:', (newRequest || request).url);
    ...
    serviceworker.addEventListener('install', function(event) {
        ...
        var onInstall = function(opts) {
            return cacheAllWithCustomHeader(opts.cacheName, opts.urlsToCache)
                .then(function() {
                    console.warn(opts.cacheName, 'serviceworker installed with version', args[1]);
    ...
    serviceworker.addEventListener('fetch', function(event) {
        var shouldHandleFetch = function(event, opts) {
            var request = event.request || {},
                ...
                criteria = {
                    ...
                    isFromMyOrigin: (url.origin===serviceworker.location.origin) || url.origin.startsWith(opts.cdnUrl),
        ...
        var shouldNetworkFirst = function(request, resourceType) {
            ...
            var url = new URL(request.url),
                ...
                criteria = {
                    contentAndNotAjaxStatic: (resourceType==='content') && !ajaxStatics,
                    hasTimeStamp: (/[&|?]_ts=(\d){13}/).test(url.search),
                    ajaxProps: url.pathname.startsWith('/_itsa_server_ajax_/props/')
                };
            console.warn('shouldNetworkFirst', url.pathname, '-->', !!Object.keys(criteria).find(function(criteriaKey) {
                return criteria[criteriaKey];
            }));
            return !!Object.keys(criteria).find(function(criteriaKey) {
                return criteria[criteriaKey];
            });
        };
        ...
        var fetchFromCache = function(request) {
            var url = new URL(request.url),
                ajaxProps = url.pathname.startsWith('/_itsa_server_ajax_\/props/'),
                indexQuestionmark, newRequest;
            console.warn('try to fetchFromCache', request.url);
            if (ajaxProps) {
                // switch the questionmark-timestamp
                // because, in the cache, there is a version a timestamp set at the server's startuptime
                indexQuestionmark = request.url.indexOf('?');
            ...
            return caches.match(newRequest || request).then(function(response) {
                ...
                console.warn('fetch from cache: ', (newRequest || request).url);
                return response;
            });
        };
        ...
        var respondNetworkFirst = function(request, resourceType, cacheKey, opts) {
            console.warn('respondNetworkFirst', request.url);
            return fetch(request).then(addToCache.bind(null, cacheKey, request))
                .catch(function() {
        ...
        var respondCacheFirst = function(request, resourceType, cacheKey, opts) {
            console.warn('respondCacheFirst', request.url);
            return fetchFromCache(request, opts).then(
                function(response) {
                    console.warn('found from cache', request.url);
                    if (shouldReloadIntoCache(request)) {
                        console.warn('reload into cache');
        ...
        var onFetch = function(event, opts) {
            ...
            if (shouldNetworkFirst(request, resourceType)) {
