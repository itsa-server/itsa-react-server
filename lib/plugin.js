...
var applyVersion = function() {
    return fs.readJson(cwd+'/package.json').then(function(packageConfig) {
...
var setMiddleware = function(server) {
    ...
    serverConnection.ext('onRequest', function (request, reply) {
        ...
        if (match) {
            ...
            //     startIndex--;
            // }
            request.path = '/'+ request.path.substr(startIndex);
...
var setRoutes = function(server, autoActivateRoutes) {
    ...
    ROUTES.push({
        ...
        handler: function(request, reply) {
            reply.file(cwdPrefix+'/assets/js/'+request.params.filename+'.js');
        },
        config: {
            cache: {
                expiresIn: EXPIRE_ONE_YEAR
    ...
    ROUTES.push({
        ...
        handler: function(request, reply) {
            reply.file(cwdPrefix+'/assets/css/'+request.params.filename+'.css');
        },
        config: {
            cache: {
                expiresIn: EXPIRE_ONE_YEAR
    ...
    // all other assets follow with as deep nested as needed:
    ROUTES.push({
        method: 'GET',
        path: '/assets/{filename*}',
        handler: function(request, reply) {
            // inert will set an eTag. We leave `no-cache` because the file might change while the name keeps the same.
            reply.file(cwdPrefix+'/assets/'+request.params.filename);
        }
    });
...
var get404File = function() {
    return new Promise(function(resolve, reject) {
        var file404 = cwd+'/src/'+FILE404;
        fs.readFile(file404).then(
...
var initServer = function(server, options) {
    ...
    server.decorate('reply', 'assets', function(filename) {
        var prefix = cwd+buildPrefix+'assets/',
            reply = this;
    ...
    server.decorate('reply', 'reactview', function(view, config) {
        ...
        beforePromise.then(function(beforeContext) {
            onlyProperties = (request.headers['x-ajaxtype']==='props');
            ...
            .then(
                function(affinityView) {
                    ...
                    if (request.headers['x-ajaxtype']==='comp') {
                        fs.readFile(cwd+'/build/assets/js/'+VIEW_COMPONENT_NRS[affinityView].hash+'.js', 'utf8')
                        .catch(
                            function() {return '';}
                        )
                        .then(
                            function(data) {
                                ...
                                reply(ATTACK_SAFE_JSON+data).ttl(EXPIRE_ONE_YEAR).etag(VIEW_COMPONENT_NRS[affinityView].hash);
                            }
                        );
                    }
                    else if (request.headers['x-ajaxtype']==='css') {
                        fs.readFile(cwd+'/build/assets/css/'+VIEW_COMPONENT_NRS[affinityView].cssfile, 'utf8')
                        .catch(
                            function() {return '';}
                        )
                        .then(
                            function(data) {
                                reply(data).ttl(EXPIRE_ONE_YEAR).etag(VIEW_COMPONENT_NRS[affinityView].cssfile);
                            }
                        );
                    }
                        ...
                        if (DEFINED_VIEWS[affinityView]===true) {
                            ...
                            if (DEFINED_VIEWS[affinityView]===undefined) {
                                fs.stat(cwd+'/build/view_components/'+affinityView+'.js').then(
                                    function() {
...
var getAffinityView = function(view, device) {
    var getView = function(level) {
        ...
        var viewFile = cwd+'/build/view_components/'+viewAffinity+'.js';
        return fs.stat(viewFile).then(
            function() {
...
var mergeAssets = function(context, affinityView) {
    var prefix = cwd+'/build/';
    var getCommonCss = function() {
        var filename;
        if (GLOBAL_ASSETS.COMMONCSS!==undefined) {
            return Promise.resolve({
                link: GLOBAL_ASSETS.COMMONCSS,
                inline: GLOBAL_ASSETS.COMMONCSSINLINE
            });
        }
        // not yet looked up: search on the disk
        // first, we need to find the right reference
        filename = prefix+'assets/css/'+VIEW_COMPONENT_COMMON.cssfile;
        return fs.stat(filename).then(
            function() {
                GLOBAL_ASSETS.COMMONCSS = '/assets/css/'+VIEW_COMPONENT_COMMON.cssfile;
                appConfig.inlinecss && (GLOBAL_ASSETS.COMMONCSSINLINE=fs.readFileSync(filename, 'utf8'));
                return {
                    link: GLOBAL_ASSETS.COMMONCSS,
                    inline: GLOBAL_ASSETS.COMMONCSSINLINE
                };
    ...
    var getPageCss = function() {
        ...
        // not yet looked up: search on the disk
        filename = prefix+'assets/css/'+VIEW_COMPONENT_NRS[affinityView].cssfile;
        return fs.stat(filename).then(
            function() {
                GLOBAL_ASSETS.CSS[affinityView] = '/assets/css/'+VIEW_COMPONENT_NRS[affinityView].cssfile;
    ...
    var getPageScript = function() {
        ...
        // not yet looked up: search on the disk
        filename = prefix+'assets/js/'+VIEW_COMPONENT_NRS[affinityView].hash+'.js';
        return fs.stat(filename).then(
            function() {
                GLOBAL_ASSETS.JS[affinityView] = '/assets/'+VIEW_COMPONENT_NRS[affinityView].hash+'.js';
    ...
    var getCommonScript = function() {
        ...
        // not yet looked up: search on the disk
        filename = prefix+'assets/js/'+VIEW_COMPONENT_COMMON.hash+'.js';
        return fs.stat(filename).then(
            function() {
                GLOBAL_ASSETS.COMMONJS = '/assets/'+VIEW_COMPONENT_COMMON.hash+'.js';
                return GLOBAL_ASSETS.COMMONJS;
            },
            function() {GLOBAL_ASSETS.COMMONJS = '';}
        );
    };

    var getYPromise = function() {
        return fs.readFile(__dirname+'/promise-min.js', 'utf8')
               .catch(function() {return '';});
    };

    var getES6Polyfill = function() {
        return fs.readFile(__dirname+'/babel-polyfill-min.js', 'utf8')
               .catch(function() {return '';});
    };
    ...
    return Promise.all(promises).then(function(response) {
        var assets = {
            __itsacommonlinkcss: response[0].link,
            __itsapagelinkcss: appConfig.inlinecss ? null : response[1].link,
            __itsapageinlinecss: appConfig.inlinecss && {__html: response[1].inline},
            __itsapagescript: response[2],
            __itsacommonscript: response[3],
            __itsapromisepolyfillscript: response[4],
            __itsapolyfilles6script: response[5]
        };
...
var applyBuildStats = function() {
    return fs.readJson(cwd+'/build/build-stats.json').then(
        function(data) {
