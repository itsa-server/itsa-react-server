...
},
{
    ...
    **/
    loadView: function(location, forceFetchComponent, cookie) {
        var instance = this,
            ...
            cleanPrevRequests, loadProps, loadCss, loadComponent, promiseRequests;

        instance._busyLoadingView = true;

        cleanPrevRequests = function() {
            // first: currently loading another io-promise: then abort!
            // this way we prevent previous loaded pages, which take more time than the new one, to reload over the latest page
        ...
        loadProps = function(cookie) {
            ...
            return instance._viewPropIO;
        };

        loadCss = function(preventCache) {
            instance._viewCssIO = io.get(origin+'/_itsa_server_ajax_/css/'+routeHash+pathname, {preventCache: !!preventCache});
            return instance._viewCssIO;
        };

        loadComponent = function(preventCache) {
            instance._viewCompIO = io.get(origin+'/_itsa_server_ajax_/comp/'+routeHash+pathname, {preventCache: !!preventCache});
            return instance._viewCompIO.then(
                function(componentCode) {
                    ...
                    if (componentCode) {
                        ...
                        // commonJS will return its Func right away,
                        // ES6 modules will return an object, where we need the default Func:
                        if (Object.itsa_isObject(BodyComponent)) {
        ...
        if (privateView && !forceFetchComponent) {
            // first request props, for this request never comes from cache
            // if it seems that the user is not authenticated, then we MUST load the css and component with a headers that marks it to be uncached
            // in order to recieve the redirected login-view
            promiseRequests = loadProps(cookie).then(function(props) {
                var redirectedToLoginPage = (props.__appProps.authentication===false);
                return Promise.all([
                    loadComponent(redirectedToLoginPage),
                    loadCss(redirectedToLoginPage),
                    Promise.resolve(props)
                ]);
            });
        }
        else {
            promiseRequests = Promise.all([
                loadComponent(forceFetchComponent),
                loadCss(forceFetchComponent),
                loadProps(cookie)
            ]);
        }

        return promiseRequests.then(
            ...
            instance._busyLoadingView = false;
            instance._reloadPropsRequest && instance.reloadProps();
        });
    ...
    **/
    getRouteFromAnchor: function(href, switchLang) {
        var controllerProps = controller.getProps(),
            view, title, questionmark, staticURI, requireId, privateView, componentId,
    ...
    _defFnNavigate: function(e) {
        ...
        instance.navigatePromise && instance.navigatePromise.reject();
        instance.navigatePromise = Promise.itsa_manage();
        if (!e.noHistoryPush) {
            if (e.newProps) {
                propsId = idGenerator('props');
                // we cannot pass through all the props with history.state, because its size is limited on FF
                PROPS_CACHE[propsId] = e.newProps;
                later(() => {
                    delete PROPS_CACHE[propsId]; // cleanup memory
                }, 5000);
            }
            try {
                instance.history.push({
                    ...
                    state: {
                        ...
                        lang: route.lang,
                        hash: route.hash,
                        newProps: propsId // when set, there won't be any ajax requests but just these new props being set
                    }
                });
            }
            catch (err) {
        ...
        return instance.navigatePromise;
    },
    ...
    **/
    clickCb: function(e) {
        var route, href, switchLang, target,
            instance = this,
            anchorNode = instance.getAnchor(e.target);
        if (anchorNode) {
            ...
            if (href && (target.toLowerCase()!=='_blank')) {
    ...
    **/
    ...
    reloadProps: function() {
        var instance = this,
            location = (instance.lastLocation || instance.initialLocation || {}).itsa_deepClone(),
            state = instance.getRouteFromAnchor(location.pathname),
            routeHash = state.routeHash,
            lang = state.lang,
            pathname = location.pathname,
            search = location.search,
        ...
        if (instance._busyLoadingView) {
            instance._reloadPropsRequest = true;
            return Promise.reject();
        }
        else {
            instance._reloadPropsRequest = false;
            if (instance._viewPropIO && instance._viewPropIO.abort) {
                instance._viewPropIO.abort();
            }
            instance._viewPropIO = io.read(origin+'/_itsa_server_ajax_/props/'+routeHash+pathname+search, null, options);
            return instance._viewPropIO.then(function(data) {
                return controller.setPageProps(data);
            })
            .then(function(changed) {
                if (changed) {
                    instance.emit('pagereloaded', {langSwitch: false});
                }
            });
        }
    },

    setPasiveUrl: function(nextUrl) {
        this.setNewProps(nextUrl, {}); // by setting an empty object as this.props, the url will be defined, yet nothing rendered
    },

    setNewProps: function(url, newProps) {
        const route = this.getRouteFromAnchor(url);
        if (route.view) {
            this.emit('navigate', {
                route: route,
                href: url,
    ...
    reloadInitialView: function() {
        this.loadView(this.initialLocation);
    },
        ...
        if (instance.isBrowserWithHistory()) {
            ...
            instance.initialLocation = {
                ...
                state: {
                    ...
                    privateView: controller.isPrivateView(),
            ...
            instance.unlistenHistory = history.listen(function(location) {
                var pathString, hashNode, path, hash, pathname, route, hashIndex, newProps, promise;
                if (!location.state) {
                    pathname = location.pathname;
                ...
                if (pathString===controller.getPath()) {
                    ...
                    newProps = location.state.newProps;
                    // newProps is currently an id which points to PROPS_CACHE[propsId]:
                    newProps && (newProps=PROPS_CACHE[newProps])
                    if (newProps) {
                        instance.lastLocation = location.itsa_deepClone(); // for usage when calling `reload`
                        promise = (!newProps || newProps.itsa_isEmpty()) ? Promise.resolve() : controller.setPageProps(newProps);
                    }
                    else if (route && (route.view===controller.getView())) {
                        instance.lastLocation = location.itsa_deepClone(); // for usage when calling `reload`
                        instance._saveHistoryHash(location.hash, location.action!=='PUSH'); // fire event without save history
                        promise = instance.reloadProps();
                        controller._path = pathString; // making sure next time "if (pathString===controller.getPath())" goes well
                    }
                    else {
                        promise = instance.loadView(location);
                    }
                }
                promise.itsa_finally(function() {
                    instance.navigatePromise && instance.navigatePromise.fulfill();
                });
            });
        }
    },
