...
var Controller = Classes.createClass(function() {
    ...
    globalstate = initialProps.globalstate;
    globalState.setInitialState(globalstate, true);
    delete initialProps.globalstate;
    instance._setProps(initialProps);
    if (initialProps.__appProps && initialProps.__appProps.ga) {
        instance._setupGA(initialProps.__appProps.ga);
    }
    instance._initCss();
},
{
    _initBodyComponent: function() {
        ...
        instance._reRender();
        globalState.onChange(instance._updateGlobalState.bind(instance));
    },

    _updateGlobalState: function() {
        var instance = this,
            globalStateDef = instance.props && instance.props.__appProps && instance.props.__appProps.globalStateDef,
            cookieGlobalstate, globalStateDefLimitedProps, keys;
        if (globalStateDef) {
            cookieGlobalstate = globalState.state;
            globalStateDefLimitedProps = globalStateDef['limited-props'];
            if (Array.isArray(globalStateDefLimitedProps) && (globalStateDefLimitedProps.length)) {
                keys = cookieGlobalstate.itsa_keys();
                keys.forEach(function(key) {
                    if (!globalStateDefLimitedProps.itsa_contains(key)) {
                        delete cookieGlobalstate[key];
                    }
                });
            }
            Cookies.set('globalstate', cookieGlobalstate, globalStateDef);
        }
    ...
    _setProps: function(props) {
        ...
        instance.props = props.itsa_deepClone();
    ...
    _reRender: function(prevView, prevCss, prevProps) {
        var instance = this,
        ...
        return new Promise(function(resolve) {
            ...
            async(function() {
                if ((prevView!==instance.view) || !prevProps.itsa_sameValue(props)) {
                    instance._createBodyElement(instance.props);
                }
    ...
    _createBodyElement: function(props) {
        ...
        if (viewContainer) {
            ...
            newProps.globalstate = globalState.state;
    ...
    setPage: function(config/* view, BodyComponent, title, props, css, componentId, requireId */) {
        var instance = this,
            prevView = instance.view,
            prevCss = instance.css,
            prevProps = instance.props;
        DOCUMENT.title = config.title || '';
        instance.BodyComponent = config.BodyComponent;
        config.props && instance._setProps(config.props);
        // specify lang AFTER props (because of the fallback)
        instance.lang = config.lang || instance.lang || (instance.props.__appProps && instance.props.__appProps.lang) || 'en';
        ...
        instance._path = WINDOW.location.pathname;
        return instance._reRender(prevView, prevCss, prevProps);
    },
    setPageProps: function(props) {
        var instance = this,
            prevProps = instance.props,
            appProps;
        if (props && props.__appProps) {
            if (prevProps.itsa_sameValue(props)) {
            ...
            console.warn('controller.setPageProps is called without the member __appProps');
        }
